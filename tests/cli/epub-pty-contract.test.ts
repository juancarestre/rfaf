...
function runEpubPty(actions: string[]): { exitCode: number; output: string } {
  const pythonScript = `
...
proc = subprocess.Popen(
    ['bun', 'run', 'src/cli/index.tsx', 'tests/fixtures/sample.epub'],
    stdin=slave,
    stdout=slave,
    stderr=slave,
    cwd='${process.cwd()}',
...
describe("epub PTY contract", () => {
  it("starts reading a local EPUB and quits cleanly", () => {
    const result = runEpubPty(["quit"]);

    expect(result.exitCode).toBe(0);
    expect(result.output).toContain("Press Space to start (RSVP)");
    expect(result.output).toContain("sample.epub");
  });
});
