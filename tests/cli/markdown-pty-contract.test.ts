...
function runMarkdownPty(actions: string[]): { exitCode: number; output: string } {
  const pythonScript = `
...
proc = subprocess.Popen(
    ['bun', 'run', 'src/cli/index.tsx', 'tests/fixtures/sample.md'],
    stdin=slave,
    stdout=slave,
    stderr=slave,
    cwd='${process.cwd()}',
...
describe("markdown PTY contract", () => {
  it("starts reading a local markdown file and quits cleanly", () => {
    const result = runMarkdownPty(["quit"]);
